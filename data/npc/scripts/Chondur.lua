...
-- Compatibility note:
-- Some datapacks for TFS 1.2 initialize the global Storage.* table AFTER NPC scripts are loaded.
-- This NPC (from a newer datapack) referenced Storage.OutfitQuest.* at load-time (as function arguments),
-- which causes: "attempt to index field 'OutfitQuest' (a nil value)".
-- We resolve those storage constants lazily (when the player actually talks to the NPC).

local function resolveStorage(value)
		...
		local yesKeyword = taskKeyword:addChildKeyword({'yes'}, StdModule.say, {npcHandler = npcHandler, text = text[2]})
			...
			yesKeyword:addChildKeyword({'yes'}, StdModule.say, {npcHandler = npcHandler, text = text[3], reset = true}, nil,
				function(player)
					...
					local ms = resolveStorage(missionStorage)
					if ms then
						player:setStorageValue(ms, 1)
					end
...
addTaskKeyword({
	...
	}, -1, function() return Storage.OutfitQuest.Shaman.MissionStaff end
)
...
addTaskKeyword({
	...
	}, 2, function() return Storage.OutfitQuest.Shaman.MissionMask end
)
...
addItemKeyword('mandrake', 'voodoo doll',
	...
	}, 1, {{itemId = 5015, count = 1}, {itemId = 3955, count = 5}}, 2, function() return Storage.OutfitQuest.Shaman.MissionStaff end
)
addItemKeyword('tribal mask', 'banana staff',
	...
	}, 3, {{itemId = 3966, count = 5}, {itemId = 3967, count = 5}}, 1, function() return Storage.OutfitQuest.Shaman.MissionMask end, true
)
