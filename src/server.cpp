...
#include "ban.h"
 ...
 // Dla make_address (nowsze Boost.Asio)
...
void ServicePort::open(uint16_t port)
{
	...
	try {
		if (g_config.getBoolean(ConfigManager::BIND_ONLY_GLOBAL_ADDRESS)) {
			// IP z configa (np. "127.0.0.1" albo publiczne). Działa też dla IPv6 jeśli wpiszesz v6.
			const auto addr = boost::asio::ip::make_address(
				g_config.getString(ConfigManager::IP)
			);

			acceptor.reset(new boost::asio::ip::tcp::acceptor(
				io_service,
				boost::asio::ip::tcp::endpoint(addr, serverPort)
			));
		}
		else {
			// Nasłuch na wszystkich interfejsach (IPv4)
			acceptor.reset(new boost::asio::ip::tcp::acceptor(
				io_service,
				boost::asio::ip::tcp::endpoint(boost::asio::ip::address_v4::any(), serverPort)
			));
		}
